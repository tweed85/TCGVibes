...
| User-choice prompts vs auto-apply | Every effect that grants a printed player choice — Drakloak Recon Directive (`peek2Top`), Cradily Selective Slime (`flipChooseStatusOpp`), Gumshoos Evidence Gathering (`swapHandCardWithDeckTop`), Mandibuzz Look for Prey (`revealOppHandPutOnOppBench`), Blissey ex Happy Switch (`moveOwnBasicEnergyBetween`), Mega Greninja ex Mortal Shuriken (`discardHandEnergyPlaceCountersOnOpp`), every Search/Recover prompt | [USER_CHOICE_AUDIT.md](USER_CHOICE_AUDIT.md) (regenerated by `npm run audit:choices`); exhaustiveness guards in [effectAuditCoverage.test.ts](../src/engine/__tests__/effectAuditCoverage.test.ts) (effect-kind audit, behavior-shape audit, unparsed-text baseline, report status ledger) | Current gate guarantees zero high/medium `autoSuspect` rows for parsed effect kinds only. Remaining `needsFix` and `unparsed` rows are intentionally visible in the report and must be converted, parsed, or excepted before claiming full tournament-pool user-choice coverage. Audit data: [_audit/userChoiceAudit.ts](../src/engine/_audit/userChoiceAudit.ts), shapes: [_audit/promptBehaviorAudit.ts](../src/engine/_audit/promptBehaviorAudit.ts). |
