...
// Predicate evaluator + AttackContext lifted into ./effects/predicate.ts
// (Stage 6A.1 mechanical extraction). Re-exported here so existing
// `import { evaluatePredicate, AttackContext } from "./effects"` callers
// (and `from "../effects"` in tests) stay unchanged.
import { evaluatePredicate, type AttackContext } from "./effects/predicate";
export { evaluatePredicate };
export type { AttackContext };

// Private cross-resolver helpers extracted to ./effects/_helpers.ts as
// part of Stage 6A.2. Used internally by resolveAttackEffects; not
// re-exported (none are part of the public engine surface).
import {
  shuffleArr,
  searchFilterMatches,
  resolveSearchDeckAttack,
  effectsPrevented,
